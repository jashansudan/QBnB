...
//add a property
    ...
    // include database connection
		...
		$propertyToDelete = $_POST['address'];


        // Insert
		$sql = "DELETE FROM rental_properties WHERE rental_properties.address = $propertyToDelete";
		...
		<center>
			<div id = "propertyform">
				<form method = "post" action = "<?php $_PHP_SELF ?>">
					...
					<input type = "submit" id = "createProperty" value = "Create Property" name = "createProperty" <a href="#">Shortcuts</a>>
				</form>
			</div>
		</center>
	</div>
		...
		<center>
			...
			<input type = "image" id = "close_property_del" src = "images/close.png">
				<ul style="list-style-type:none">
					<?php
					foreach ($result as $row) { ?> 
					 <li><input type="submit" id="delete" name="delete" class="btn btn-md btn-primary btn-block smallMargin">  <?= $row["address"] ?> </button> </li>
					<?php }

					$propertyToDelete = $row["address"];
					 ?>
				</ul>
				</form>
			</div>
